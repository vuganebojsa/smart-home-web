...
class RegularUser(HttpUser):
    ...
    @task
    def get_report_for_sps_from_to_username(self):
        headers = {"Authorization": f"Bearer {self.token}"}
        ...
        url = f"/reports/solar-panel-history/{sps.get('id')}?username=nebojsavuga&from={from_date_str}&to={to_date_str}"
    ...
    @task
    def get_report_for_sps_username(self):
        headers = {"Authorization": f"Bearer {self.token}"}
        ...
        url = f"/reports/solar-panel-history-username/{sps.get('id')}?username=nebojsavuga"
        response = self.client.get(url, headers=headers)
