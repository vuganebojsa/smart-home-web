...
class RegularUser(HttpUser):
    ...
    #     }
    #     headers = {"Authorization": f"Bearer {self.token}"}
    #     response = self.client.post("/smartDevices/register-vehicle-gate", json=payload, headers=headers)
    #     if response.status_code != 200:
    #         print("Greška prilikom kreiranja Kapije")
    ...
    def create_sprinkler(self):
        ...
        headers = {"Authorization": f"Bearer {self.token}"}
        response = self.client.post("/smartDevices/register-sprinkler-system", json=payload, headers=headers)
        if response.status_code != 200:
            print("Greška prilikom kreiranja Prskalica")